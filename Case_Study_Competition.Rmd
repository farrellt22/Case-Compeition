...
|      Shows the accuracy that our logistic regression model has in predicting Yes and no Responses. The sensitivity of our model shows the accuracy we have for predicting "yes" responses and the specificity of our model shows the accuracy we have in predicting "no" responses. This model has an accuracy of 58.98%. We used .51 as our optimal cutoff point which gave us the best sensitivity and specificity which shows us that our model was accurate at predicting both yes and no responses.

```{r}
a1 = glm(formula = Donation ~ NeighborhoodCode + PercJapa + PercChin + AvgRent + IC3H + LFAF + MaxDol + LastDol, data = balancedb1 , family = binomial)
...
```{r}
optim_threshold(a1,balancedb1, balancedb1$Donation)
```
...
```{r}
rftrain =randomForest(Donation~NeighborhoodCode + PercJapa + PercChin + AvgRent + IC3H + LFAF + MaxDol + LastDol, data = balancedb1)
```

```{r}
confusionMatrix(predict(rftrain,b1test), b1test$Donation, positive = '1')
```
...
```{r}
m1.lda = lda(formula = as.factor(Donation) ~ NeighborhoodCode + PercJapa + PercChin + AvgRent + IC3H + LFAF + MaxDol + LastDol, data = balancedb1)
